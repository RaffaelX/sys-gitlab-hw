...
provider "yandex" {
  ...
  cloud_id                 = "b1g88v2kfqjkhtjlb090"
  folder_id                = "b1gu6c45gvfp9tc7optc"
}
...
resource "yandex_compute_instance" "zabbix-server" {
  ...
  name                      = "linux-zabbix-server"
  allow_stopping_for_update = true
  platform_id               = "standard-v2"
  zone                      = "ru-central1-c"
  hostname                  = "linux-zabbix-server"
...
output "external_ip_address_bastion-host" {
